...
def self_check_system():
    ...
    if not API_URL.startswith("http"):
        ...
        print("  -> Format required: A valid URL like 'http://localhost:11434/api/generate'")
...
class OllamaClient:
    ...
    def generate(self, prompt: str, temperature: float = 0.7) -> str:
        payload = {"model": self.model_name, "prompt": prompt, "stream": False, "options": {"temperature": temperature}}
        data = json.dumps(payload).encode("utf-8")
        req = request.Request(self.api_url, data=data, headers={"Content-Type": "application/json"}, method="POST")
...
class SymbioticAI:
    ...
    def add_memory(self, category: str, content: str, importance: float = 0.5):
        self.episodic_memory.append(MemoryItem(timestamp=time.time(), category=category, content=content, importance=importance))
    ...
    def summarize_memory(self, limit: int = 8) -> str:
        ...
        items = sorted(self.episodic_memory, key=lambda x: x.importance, reverse=True)[:limit]
...
""".strip()
    ...
    def build_revision_prompt(self, original_prompt: str, draft: str, scores: Dict[str, int]) -> str:
...
def run_cognitive_turn(sym_ai: SymbioticAI, client: OllamaClient, user_input: str):
    ...
    sym_ai.add_memory("interaction", f"User asked: {user_input[:50]}...", importance=0.7)
    print(f"\n[MOUSE]: {draft}")
