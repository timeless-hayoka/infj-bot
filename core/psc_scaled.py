...
def _batch_linear_predict(
    history_matrix: np.ndarray, n_steps: int
...
def _batch_ewma_predict(
    history_matrix: np.ndarray, alphas: np.ndarray, n_steps: int
) -> tuple[np.ndarray, np.ndarray]:
    """
...
def _batch_adaptive_alpha(
    history_matrix: np.ndarray, chaos_scores: np.ndarray
) -> np.ndarray:
    """
...
class PSCBatchEngine:
    ...
    def run(self) -> Optional[PSCBatchResult]:
        ...
        H = np.stack(
            [h[-min_len:].astype(np.float64) for h in histories], axis=1
        )  # (T, D)
        T, D = H.shape
        ...
        min_conf_arr = np.where(
            chaos >= CHAOS_SCORE_HIGH, MIN_CONF_CHAOS, MIN_CONF_NORMAL
        )

        # Already in crisis — mask out (homeostasis handles those)
        in_crisis = ((self._polarities < 0) & (current <= self._thresholds)) | (
...
def apply_dmu_diversity_floor(
    candidates: list[dict], max_frac: float = DMU_MAX_CATEGORY_FRAC
) -> list[dict]:
    if not candidates:
        return candidates
...
def dmu_weighted_score(
    ...
    return float(
        np.exp(-t_elapsed / tau) * reinforcement * contextual_salience * base_sim
    )
...
def benchmark_scale(
    dim_counts: list[int] = [16, 50, 100, 200, 500], n_cycles: int = 1000
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(
            "  Chaos scores (top 3): "
            + ", ".join(
                f"{d}={result.chaos_scores[list(dims).index(d)]:.3f}"
                for d, _ in alerts[:3]
            )
        )
    ...
    for D, r in bench.items():
        print(
            f"  {D:>6} {r['mean_us']:>10.1f} {r['p50_us']:>10.1f} "
