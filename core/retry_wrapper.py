...
def self_check():
    ...
    test_cases = [
        ...
        (1000, 60),  # 20 + 25 = 45 → floor 60
        (4233, 120),  # 20 + (4*25) = 120
        (10000, 270),  # 20 + (10*25) = 270
    ]
    ...
    for length, expected in test_cases:
        ...
        if not ok:
            all_pass = False
        print(
            f"  {length:6d} chars → {result:3d}s (expected {expected:3d}s) "
    ...
    if not std_ok:
        all_pass = False
    print(
        f"  Timeout: {config_std['timeout']}s (expected {expected_std_timeout}s) "
