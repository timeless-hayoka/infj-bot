...
def self_check_diagnostics():
    if VAULT_SECRET == "default_dev_secret_do_not_use_in_prod":
        print("[⚠️ WARNING] DRIFT_VAULT_SECRET is using insecure default.")
...
class SvalbardVault:
    ...
    def _get_latest_hash_from_disk(self) -> str:
        ...
        try:
            with open(VAULT_PATH, 'rb') as f:
                ...
                file_size = f.tell()
                if file_size == 0: return "0" * 64
                
                chunk_size = 8192
                f.seek(max(0, file_size - chunk_size))
                chunk = f.read()
                lines = chunk.split(b'\n')
                last_line_bytes = lines[-1] if lines[-1].strip() else lines[-2] if len(lines) > 1 else lines[-1]
    ...
    def _sign_root_hash(self, root_hash: str):
        signature = hmac.new(VAULT_SECRET.encode('utf-8'), root_hash.encode('utf-8'), hashlib.sha256).hexdigest()
    ...
    def verify_identity_integrity(self, full_chain: bool = False) -> bool:
        if os.path.exists(SIG_PATH):
            with open(SIG_PATH, 'r') as f: saved_sig = f.read().strip()
            expected_sig = hmac.new(VAULT_SECRET.encode('utf-8'), self.latest_hash.encode('utf-8'), hashlib.sha256).hexdigest()
            if saved_sig != expected_sig:
                print("[CRITICAL SECURITY FAILURE] Signature mismatch — ledger tampered!")
                return False
        if not full_chain or not os.path.exists(VAULT_PATH): return True
        ...
        with open(VAULT_PATH, 'r') as f:
            for line_num, line in enumerate(f):
                ...
                if block["prior_hash"] != current_prior: return False
