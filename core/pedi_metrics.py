...
class PEDIEngine:
    ...
    def _get_identity_center_of_gravity(self) -> AnchorResult:
        ...
        def _is_degenerate(block) -> bool:
            ...
            return (
                emotions.get("coherence", 0.0) < 1e-6
                and emotions.get("resonance", 0.0) < 1e-6
            )

        usable = [
            b
            for b in recent_blocks
            if not b.get("quarantined", False) and not _is_degenerate(b)
        ]
    ...
    def calculate_distance(self, state_a: dict, state_b: dict) -> float:
        sum_sq = sum(
            (state_a.get(dim, 0.5) - state_b.get(dim, 0.5)) ** 2 for dim in DIMS
        )
    ...
    def evaluate_cycle(self, raw_active_state: dict):
        ...
        for k in DIMS:
            self.perceived_state[k] = (
                self.perceived_state.get(k, 0.5) * (1 - perception_weight)
            ) + (raw_active_state.get(k, 0.5) * perception_weight)
        ...
        if applied_correction > 0.05:
            for k in DIMS:
                self.perceived_state[k] = (
                    self.perceived_state.get(k, 0.5) * (1 - applied_correction)
                ) + (anchor[k] * applied_correction)
