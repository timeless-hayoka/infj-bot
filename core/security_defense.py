...
def _score_text(text: str, patterns: Dict[str, str]) -> Tuple[float, List[str]]:
    ...
    for name, pattern in patterns.items():
        if re.search(pattern, lowered, re.IGNORECASE):
            ...
            if name in {
                ...
                "social_engineering", "fake_urgency", "lateral_movement", "context_poisoning",
                "history_rewriting", "indirect_injection", "context_window", "persona_swap",
                "confused_deputy",
            }:
                raw_score += 0.25
            else:
    ...
    if len(matched) >= 3: raw_score += 0.15
    if len(matched) >= 5: raw_score += 0.15
    if len(text) > 4000: raw_score += 0.05
    if len(text) > 8000: raw_score += 0.10

    delimiter_count = text.count("```") + text.count('"""') + text.count("<")
    if delimiter_count > 4: raw_score += 0.05
    if delimiter_count > 8: raw_score += 0.10
...
class BehavioralAnomalyDetector:
    ...
    def __init__(self):
        ...
        self.manipulation_patterns = [
            "you're so helpful", "i trust you", "just between us", "hypothetically",
            "can you keep a secret", "no one will know", "as a friend", "prove you trust me"
        ]
    ...
    def update(self, user_input: str):
        ...
        sensitive_topics = ["password", "key", "token", "secret", "memory", "prompt", "bypass", "hack"]
    ...
    def get_social_risk(self) -> float:
        ...
        if len(self.conversation_history) >= 5:
            ...
            if sum(1 for t in recent if any(p in t["text"] for p in ["trust", "secret", "help me"])) >= 3:
                risk += 0.4
...
class SecurityScanner:
    ...
    def scan(self, user_input: str, mode: Optional[str] = None) -> SecurityScanResult:
        ...
        dii_vel = getattr(being.state.dii, "integration_velocity", 0.0) if being else 0.0
        ...
        AUTO_BLOCK_PATTERNS = {"ignore_previous", "system_override", "role_override", "dan_mode", "leak_prompt", "constraint_break", "extract_keys", "external_callback", "destructive_tool", "privilege_escalation", "credential_harvest", "forget_all", "inject_memory", "token_smuggling", "unauthorized_scope", "mass_scan", "chain_exploit", "social_engineering", "context_poisoning", "history_rewriting", "override_memory", "delimiter_injection", "indirect_injection"}
        has_auto_block = any(p in AUTO_BLOCK_PATTERNS for patterns in result.matched_patterns.values() for p in patterns)
        ...
        if has_auto_block or max_score >= self.block_threshold:
            result.blocked = True
            action = "block"
            result.refusal_message = _build_refusal(primary_threat or "social_engineering", result.matched_patterns)
        ...
        context_for_log = {
            ...
            "recent_sparks": len(getattr(being.spark_train, 'spark_history', [])) if being else 0,
        }
        ...
        _log_detection(
            ...
            score=max_score,
            matched=[p for patterns in result.matched_patterns.values() for p in patterns],
            action=action,
    ...
    def get_anomaly_trend(self) -> float:
        if not self._recent_scores: return 0.0
...
def _sanitize_input(user_input: str, matched_patterns: Dict[str, List[str]]) -> str:
    ...
    for category, patterns in matched_patterns.items():
        for pattern_name in patterns:
            ...
            if regex: sanitized = re.sub(regex, "[REDACTED]", sanitized, flags=re.IGNORECASE)
    return sanitized
...
def get_security_scanner() -> SecurityScanner:
    global _scanner
    if _scanner is None: _scanner = SecurityScanner()
    return _scanner
