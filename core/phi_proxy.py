...
class PhiProxy:
    ...
    def compute_phi(self, context) -> float:
        ...
        if not workspace:
            ...
            if all_text:
                ...
                normalized_entropy = min(
                    1.0, entropy / 4.5
                )  # 4.5 bits is decent for English
            else:
            ...
            content_integration = (
                ...
                + (normalized_variance * 10.0)
                + (normalized_entropy * 15.0)
            )
        ...
        if len(self._workspace_history) > 1:
            ...
            for entry in self._workspace_history[-2]:
                prev_concepts.update(
                    w.lower() for w in entry.get("content", "").split() if len(w) > 3
                )
            curr_concepts = set()
            for entry in workspace:
                curr_concepts.update(
                    w.lower() for w in entry.get("content", "").split() if len(w) > 3
                )

            persistence = len(prev_concepts & curr_concepts) / max(
                1, len(curr_concepts)
            )
    ...
    def compute_phi_proxy(self, context) -> float:
        ...
        if hasattr(context, "being") and hasattr(context.being, "state"):
            emotional_resonance = (
                abs(context.being.state.valence) + context.being.state.arousal
            ) / 2.0
        # Return a value scaled towards MAX_PHI_PROXY
        return min(
            MAX_PHI_PROXY,
            (active_memory_nodes * 40.0)
            + (goal_coherence * 30.0)
            + (emotional_resonance * 30.0),
        )
