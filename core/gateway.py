...
class HardenedGatewayMiddleware:
    ...
    def _verify_key(self, raw_key: str) -> Optional[str]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT tier FROM keys WHERE key_hash = ? AND active = 1", (key_hash,)
        )
        row = cursor.fetchone()
        conn.close()
    ...
    async def __call__(self, scope, receive, send):
        ...
        if content_length:
            try:
                if int(content_length) > MAX_PAYLOAD_BYTES:
                    logger.warning(
                        f"Payload too large from {client_ip} on {path} ({content_length} bytes)"
                    )
                    response = JSONResponse(
                        {
                            "error": "Payload Too Large",
                            "message": f"Maximum payload size is {MAX_PAYLOAD_BYTES} bytes.",
                        },
                        status_code=413,
                    )
            ...
            while more_body:
                ...
                if message["type"] == "http.request":
                    ...
                    if len(body_bytes) > MAX_PAYLOAD_BYTES:
                        logger.warning(
                            f"Payload too large from {client_ip} on {path} during stream"
                        )
                        response = JSONResponse(
                            {
                                "error": "Payload Too Large",
        ...
        if is_prod or has_keys_configured:
            ...
            if not sig_header or not ts_header:
                ...
                response = JSONResponse(
                    {
                        "error": "Unauthorized",
                        "message": "Missing X-Drift-Signature or X-Drift-Timestamp header.",
                    },
                    status_code=401,
                )
            ...
            try:
                ...
                try:
                    ...
                    response = JSONResponse(
                        {
                            "error": "Unauthorized",
                            "message": "Invalid X-Drift-Timestamp format.",
                        },
                        status_code=401,
                    )
                    await response(scope, custom_receive, send)
            ...
            if abs(now - req_ts) > 10.0:
                logger.warning(
                    f"Out of window timestamp from {client_ip} on {path} (diff: {now - req_ts:.2f}s)"
                )
                response = JSONResponse(
                    {
                        "error": "Unauthorized",
                        "message": "Request timestamp is out of the valid window (±10s).",
                    },
                    status_code=401,
                )
                await response(scope, custom_receive, send)
            ...
            if nonce_header:
                if not await state_manager.validate_nonce(nonce_header, req_ts):
                    logger.warning(
                        f"Replay attack detected (used nonce: {nonce_header}) from {client_ip}"
                    )
                    response = JSONResponse(
                        {
                            "error": "Unauthorized",
                    ...
                    response = JSONResponse(
                        {
                            ...
                            "message": "Cryptographic key configuration error.",
                        },
                        status_code=500,
                    )
                    await response(scope, custom_receive, send)
            ...
            if not hmac.compare_digest(sig_header, expected_sig):
                logger.warning(
                    f"Signature mismatch for request {method} {path} from {client_ip}"
                )
                response = JSONResponse(
                    {
                        "error": "Unauthorized",
        ...
        if not await bucket.acquire():
            logger.warning(
                f"Rate limit hit on key {key_prefix} ({tier}) from {client_ip} on {path}"
            )
            response = JSONResponse(
                {
        ...
        if not state_manager.increment_daily_usage(key_hash, tier_config["daily_cap"]):
            logger.warning(
                f"Daily quota hit on key {key_prefix} ({tier}) from {client_ip}"
            )
            response = JSONResponse(
                {
                    "error": "Too Many Requests",
        ...
        try:
            ...
            await asyncio.wait_for(
                state_manager.global_sem.acquire(), timeout=QUEUE_TIMEOUT_SECONDS
            )
        except asyncio.TimeoutError:
            logger.error(
                f"Concurrency queue timeout reached for key {key_prefix} on {path}"
            )
            response = JSONResponse(
                {
                    "error": "Too Many Requests",
                    "message": "Server concurrency queue full. Please try again later.",
                },
                status_code=429,
            )
            await response(scope, custom_receive, send)
