...
def cmd_ask(args):
    ...
    if not prompt:
        print("Give me something to ask. Example: infj_bot ask what should I focus on today?")
        return 2

    from config import DEFAULT_AUTHORIZED_TARGETS
    state = BotState(mode=args.mode, proactive_enabled=False, authorized_targets=set(DEFAULT_AUTHORIZED_TARGETS))
    brain = InfjBrain()
    memory = InfjMemory()
    history = ChatHistory()
    goals_db = GoalsDB()
    ...
    built_prompt, emotion, dissonance = build_chat_prompt(
        prompt, state, memory, goals_db=goals_db, doc_store=doc_store, tools_enabled=not args.no_tools, prefs=state.prefs
    )
    try:
...
def build_parser():
    ...
    ask.add_argument("--mode", default="companion", choices=[
        "companion", "engineer", "critic", "coach", "clarity", "researcher", "bughunter", "drift", "quiet"
    ])
    ask.add_argument("--no-save", action="store_true", help="Do not save this one-shot exchange to memory/history.")
    ask.add_argument("--no-tools", action="store_true", help="Disable tool-use for this one-shot.")
    ...
    health.add_argument("--live", action="store_true", help="Also call the configured Gemini model once.")
