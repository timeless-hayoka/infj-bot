...
def test_spark_v2_full_flow():
    ...
    print(
        "Shadow influence in last spark:",
        spark.get("shadow_influence") if spark else "N/A",
    )
    print(
        "Veto count in history:",
        sum(1 for s in b.spark_train.spark_history if s.get("vetoed")),
    )
    ...
    for i in range(5):
        ...
        if spk:
            print(
                f"Spark {i + 1} result: {spk.get('signal')} - Vetoed: {spk.get('vetoed')}, Reason: {spk.get('veto_reason')}"
            )
