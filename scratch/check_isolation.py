...
def self_check_data_isolation():
    ...
    try:
        ...
        if result is None:
            print(
                "[OK] Complete isolation confirmed. Session B cannot see Session A's goals."
            )
        else:
            print("[ERROR] Data bleed detected! Session B read Session A's data.")
            print(
                "Fix: Ensure self.db_path is strictly used in all SQL executions inside goals.py."
            )
        ...
        if brain_a.evaluator != brain_b.evaluator:
            ...
            print(
                "Fix: Check the initialization logic in brain.py to ensure 'evaluator=None' defaults correctly."
            )
