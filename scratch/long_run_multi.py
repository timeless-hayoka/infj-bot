...
def run_single_simulation(run_id: str, cycles: int = 500, test_type: str = "control"):
    ...
    for i in range(1, cycles + 1):
        ...
        if i % 50 == 0:
            print(
                f"[{run_id}] Day {i}/{cycles} | PEDI: {pedi_val:.3f} | DII: {dii_val:.3f} | Energy: {energy:.3f} | Mem: {count} | Shadow: {shadow_depth:.3f}"
            )
...
def plot_results(all_results):
    ...
    for run_id, history in all_results.items():
        ...
        axs[2].plot(
            days, energy, label=f"Energy {label}", alpha=alpha, linewidth=linewidth
        )
        axs[3].plot(
            days,
            shadow,
            label=f"Shadow Depth {label}",
            alpha=alpha,
            linewidth=linewidth,
        )
        ...
        for run_id, test_type in runs:
            futures.append(
                (
                    run_id,
                    pool.apply_async(
                        run_single_simulation, (run_id, CYCLES, test_type)
                    ),
                )
            )
