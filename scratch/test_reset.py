...
def main():
    ...
    output_1 = brain.agent_turn(
        prompt_1, tools_enabled=True, raw_user_input=message_1, mode=state.mode
    )
    ...
    output_2 = brain.agent_turn(
        prompt_2, tools_enabled=True, raw_user_input=message_2, mode=state.mode
    )
    ...
    if os.path.exists(log_path):
        with open(log_path, "r") as f:
            ...
            for idx, line in enumerate(lines, 1):
                ...
                print(
                    f"Turn {idx} logged: turn={data.get('turn')} prompt_length={data.get('prompt_length')} response_length={data.get('response_length')}"
                )
    else:
