...
OUTPUT_DIR = Path("/home/crexs/drift/ABLATION_RESULTS/autonomous_run_20260605_092804")
...
current_provider_override = (
    None  # None means use default (Gemini), "ollama" means forced Ollama
)
...
def apply_stubs(condition, orchestrator, memory):
    global \
        original_reflect, \
        original_coord_format, \
        original_shadow_format, \
        original_shadow_radar, \
        original_shadow_state, \
        original_homeo_format, \
        original_homeo_needs, \
        original_retrieve_ranked
    ...
    if condition == "A":
        ...
        try:
            ...
            if shadow:
                ...
                if hasattr(shadow, "_state") and shadow._state:
                    original_shadow_state = (
                        shadow._state.depth,
                        shadow._state.dominant_archetype,
                    )
        ...
        if not shadow_found:
            try:
                ...
                shadow.radar = {}
                print(
                    "  - Stubbed shadow format_prompt_snippet and radar via direct import"
                )
                shadow_found = True
            except Exception as e:
        ...
        try:
            ...
            for name, need in homeo.needs.items():
                ...
                need.trend = 0.0
            print(
                "  - Stubbed homeostasis format_prompt_snippet and needs flattened to 0.5"
            )
        except Exception as e:
        ...
        try:
            ...
            memory.retrieve_context_ranked = lambda query, n_results=5: (
                memory.retrieve_context(query, n_results=n_results)
            )
...
def restore_stubs(condition, orchestrator, memory):
    ...
    if condition == "A":
        ...
        if shadow:
            ...
            if (
                hasattr(shadow, "_state")
                and shadow._state
                and original_shadow_state is not None
            ):
                shadow._state.depth, shadow._state.dominant_archetype = (
                    original_shadow_state
                )
        else:
            try:
        ...
        for idx, user_prompt in enumerate(PROMPTS):
            ...
            if cond_code == "D":
                ...
                if assembled_prompt == baseline_flight:
                    ...
                    print(
                        f"    -> D_CONTROL: prompts differ! Diff len={len(d_diff_content)}"
                    )
            ...
            try:
                ...
                if (
                    ...
                    or "429" in response_text.lower()
                    or "resource_exhausted" in response_text.lower()
                ):
                    if current_provider_override != "ollama":
                        print(
                            "    [WARNING] Quota limit detected in response! Switching to Ollama..."
                        )
...
def generate_findings_report(results):
    ...
    try:
        with open(
            "/home/crexs/drift/ABLATION_RESULTS/autonomous_run_20260605_092804/trajectory_analysis.txt",
            "r",
        ) as f:
    ...
    for r in summary_rows:
        lines.append(
            f"| {r['code']:9} | {r['avg_prompt_len']:13} | {r['delta_abs']:10} | {r['delta_pct']:.2f}% | {r['verdict']:8} |"
        )
    lines.append("")
    ...
    if not d_diffs:
        lines.append(
            "Flight A and B produced identical prompts. DMU was not active for these prompts during flight comparison."
        )
    else:
        lines.append(
            f"Flight A and B produced DIFFERENT prompts for {len(d_diffs)}/5 prompts."
        )
    ...
    lines.append(
        f"- **Confirmed Load-Bearing Subsystems:** {', '.join(load_bearing) or 'None'}"
    )
    ...
    if "A" in load_bearing:
        lines.append(
            "- **Condition A (Council):** The Council of Seven (coordination/consensuses) was successfully cleared from the prompt assembly layer. The delta represents the size of active consensus and deliberation blocks."
        )
    if "B" in load_bearing:
        lines.append(
            "- **Condition B (Shadow):** The Shadow archetype intensities and text logs were ablated from the prompt. We saw a direct prompt size decrease matching the shadow's system representation."
        )
