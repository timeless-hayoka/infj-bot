...
def restore_state():
    ...
    for f in TARGET_FILES:
        ...
        if backup_file.exists():
            ...
            # Verify restoration
            assert hash_file(backup_file) == hash_file(f), (
                f"Hash mismatch on {f.name} restoration!"
            )
...
def analyze_telemetry():
    ...
    if len(set(drains)) == 1 and len(turns) > 1:
        findings["coupling_verdict"] = (
            "FLAT - Drain is identical regardless of length. BASE_NETWORK_COST is doing all the work."
        )
    else:
        ...
        findings["coupling_verdict"] = (
            f"VARIANCE DETECTED (r={r:.3f}). Proceed to strict alpha calibration."
        )
    ...
    print(
        f"[+] Calibration log copied to {RESULTS_DIR / 'test_only_governor_calibration.jsonl'}"
    )
        ...
        for line in sys.stdin:
            try:
                ...
                if data.get("event") == "TURN":
                    print(
                        f"[Turn {data.get('turn')}] Energy: {data.get('new_energy'):.3f} (Δ {data.get('delta_energy'):.4f}) | Mode: {data.get('energy_mode')} | Gated: {data.get('gate_applied')}"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--monitor",
        action="store_true",
        help="Run in live monitor mode reading from stdin",
    )
