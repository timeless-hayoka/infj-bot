...
def invoke(host: str, port: int, tool: str, args=None, kwargs=None, token: str | None = None) -> Dict[str, Any]:
...
def main() -> int:
    ...
    p.add_argument("--run-all", action="store_true", help="Run a set of demo invocations")
    args = p.parse_args()
    ...
    try:
        ...
        if args.run_all:
            demos = [
                ...
                ("dissonance_map", {"args": ["I want to change careers but I'm afraid of losing stability."]}),
            ...
            for tool, payload in demos:
                print(f"-> {tool}")
                out = invoke(args.host, args.port, tool, payload.get("args"), payload.get("kwargs"), token=token)
