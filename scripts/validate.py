...
def conversational_run(cycles: int, delay: float = 2.0) -> List[Dict]:
    ...
    for i in range(cycles):
        ...
        status = (
            "TIMEOUT" if record["timed_out"] else f"{record['latency_seconds']:.2f}s"
        )
        phi_val = "N/A"
        ...
        if record.get("phi") and "needs" in record["phi"]:
            n = record["phi"]["needs"]
            e = (
                n.get("energy", {}).get("level", 0)
                if isinstance(n.get("energy"), dict)
                else n.get("energy", 0)
            )
            c = (
                n.get("coherence", {}).get("level", 0)
                if isinstance(n.get("coherence"), dict)
                else n.get("coherence", 0)
            )
            itg = (
                n.get("integration", {}).get("level", 0)
                if isinstance(n.get("integration"), dict)
                else n.get("integration", 0)
            )
            needs = f"E:{e:.2f} C:{c:.2f} I:{itg:.2f}"

        print(
            f"[CONV] cycle {int(i + 1):3d}/{cycles} | latency {status:>8s} | phi {phi_val:>6s} | needs {needs}",
            flush=True,
        )
...
def stress_run(cycles: int) -> List[Dict]:
    ...
    for i in range(cycles):
        ...
        print(
            f"[STRESS] cycle {int(i + 1):3d}/{cycles} | latency {status:>8s} | phi {phi_val:>6s} | needs {needs}",
            flush=True,
        )
    return results
...
def summarize(results: List[Dict]) -> Dict:
    ...
    latencies = [
        r["latency_seconds"]
        for r in results
        if not r["timed_out"] and not r.get("http_error")
    ]
    ...
    return {
        ...
        "success_rate": round((total - timeouts - http_errors) / total, 3)
        if total
        else 0,
...
def main():
    ...
    parser.add_argument(
        "--cycles", type=int, default=20, help="Number of cycles to run"
    )
    parser.add_argument(
        "--mode",
        choices=["conversational", "stress"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Delay between cycles in conversational mode",
    )
    ...
    if health is None or "error" in health:
        ...
        sys.exit(1)
    print(
        f"[+] Server OK: turns={health.get('turns', '?')}, memory={health.get('memory_count', '?')}, mode={health.get('mode', '?')}"
    )
    print("")
    ...
    print(
        f"Degradation point: cycle {summary['degradation_point']}"
        if summary["degradation_point"]
        else "Degradation point: none detected"
    )
    if summary["phi_values"]:
        print(
            f"Phi range:         {min(summary['phi_values']):.2f} — {max(summary['phi_values']):.2f}"
        )
    if summary["energy_values"]:
        print(
            f"Energy range:      {min(summary['energy_values']):.2f} — {max(summary['energy_values']):.2f}"
        )
    print("")
