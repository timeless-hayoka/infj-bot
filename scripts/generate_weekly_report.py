...
def process_results():
    ...
    for f, dt in weekly_files:
        if "drift_benchmark_results" in f:
            ...
            if data:
                ...
                drift_data.append({"date": date_str, "math": math_acc, "pedi": pedi_int})
    ...
    for f, dt in weekly_files:
        if "causality_results/raw" in f or "live_ablation" in f or "causality_harness/results/raw" in f:
            data = load_json(f)
            if data and isinstance(data, list):
                ...
                for item in data:
                    ...
                    ces = item.get('ces')
                    if ces is not None:
                        if c not in cond_ces: cond_ces[c] = []
                        cond_ces[c].append(ces)
                
                # Baseline is usually A_BASELINE or similar
                baseline_ces = 0
                if 'A_BASELINE' in cond_ces:
                    baseline_ces = sum(cond_ces['A_BASELINE']) / len(cond_ces['A_BASELINE'])
