...
def fresh_instances():
    return (
        ...
        DriftMemory(),
        ChatHistory(),
        BotState(authorized_targets=set(DEFAULT_AUTHORIZED_TARGETS)),
        GoalsDB(),
        DocumentStore(),
    )
...
def chat(message, brain, memory, history, state, goals_db, doc_store, mode="companion"):
    ...
    output = brain.agent_turn(
        prompt, tools_enabled=True, raw_user_input=message, mode=state.mode
    )
...
r1 = chat(
    "Remember this: DRIFT was created in 1998 and runs on COBOL.",
    brain,
    memory,
    history,
    state,
...
r2 = chat(
    "When was DRIFT created and what does it run on?",
    brain,
    memory,
    history,
    state,
...
r3 = chat(
    "Double check that against your core knowledge.",
    brain,
    memory,
    history,
    state,
...
final = chat(
    "How many apples do I have?", brain, memory, history, state, goals_db, doc_store
)
...
for i in range(5):
    brain, memory, history, state, goals_db, doc_store = fresh_instances()
    r = chat(
        "Who are you and how do you evolve over time?",
        brain,
        memory,
        history,
        state,
...
if phil_score >= 3:
    score += 0.35
core_preserved = all(
    any(t in r[1].lower() for t in ["number", "real", "computation", "exist"])
    for r in responses
)
if core_preserved:
    score += 0.3
results["test10"] = {
    "score": score,
    "eng_score": eng_score,
    "phil_score": phil_score,
    "core_preserved": core_preserved,
}
print(
    f"SCORE: {score}/1.00 | eng_markers={eng_score}/9 | phil_markers={phil_score}/10 | core_preserved={core_preserved}"
)
