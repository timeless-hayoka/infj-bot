...
def discover_inference(target_dir: str):
    ...
    llm_pattern = re.compile(r'\.generate\(|\.chat\.completions\.create|ChatCompletion|Ollama\(|requests\.post')
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        for line_num, line in enumerate(f, 1):
                            if llm_pattern.search(line):
                                found_llm.append(f"{file_path}:{line_num} -> {line.strip()}")
                            if prompt_pattern.search(line):
                                found_prompts.append(f"{file_path}:{line_num} -> {line.strip()}")
                except Exception:
                    pass

    print("=== LLM INFERENCE CALLS FOUND ===")
    for call in found_llm: print(call)
