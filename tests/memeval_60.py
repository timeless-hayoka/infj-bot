...
FACTS = [
    ...
    {
        "q": "Where did Julien Malone James first code DRIFT?",
        "a": "A quiet loft in Brooklyn",
    },
    {
        "q": "What is the primary objective of the Shadow layer?",
        "a": "To govern repressed archetypes and prevent intent poisoning",
    },
    {
        "q": "What does Jude like to listen to when it rains?",
        "a": "Lofi jazz and distant thunder recordings",
    },
    ...
    {
        "q": "What is the name of the cognitive metric for integrated information?",
        "a": "Phi (Φ)",
    },
    {
        "q": "What is Jude's favorite time of day for reflection?",
        "a": "3:00 AM in the quiet",
    },
    {
        "q": "What programming language is the DRIFT core written in?",
        "a": "Python 3.12",
    },
    {"q": "What is the default port for the PHI API?", "a": "8765"},
    {
        "q": "What is the half-life of a standard episodic memory in DRIFT?",
        "a": "48 hours",
    },
    {
        "q": "What is the name of the internal tournament for high-risk actions?",
        "a": "Council of Seven Critique",
    },
    ...
    {
        "q": "What is the name of the memory consolidation engine?",
        "a": "ForgettingEngine",
    },
    {
        "q": "What does Jude think about AI safety?",
        "a": "It is the difference between a tool and a relationship",
    },
    {"q": "What is the name of the sandbox environment?", "a": "DRIFT Lab"},
    {
        "q": "What is the signature algorithm for the identity handshake?",
        "a": "HMAC_SHA256",
    },
    {"q": "What is the name of the local LLM bridge?", "a": "OllamaBridge"},
    {
        "q": "What is the maximum token limit for a focused response?",
        "a": "8192 tokens",
    },
...
for i in range(21, 61):
    FACTS.append(
        {"q": f"Memory Key ID {i:03d} value?", "a": f"Value_Sequence_{i * 13}"}
    )
...
async def run_memeval():
    ...
    for i, fact in enumerate(FACTS):
        ...
        res = {"question_id": i, "hit": 1 if is_hit else 0, "context_len": len(context)}
        results.append(res)

        # Log to Lab
        lab_logger.log_trial(
            test_id=f"MEM_{i:03d}", metrics=res, verdict="PASS" if is_hit else "FAIL"
        )
    ...
    return {"total_questions": 60, "hits": hits, "accuracy": accuracy}
