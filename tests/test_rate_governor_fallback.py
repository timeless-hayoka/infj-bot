...
class TestRateGovernorFallback(unittest.TestCase):
    def test_run_chain_accumulates_errors(self):
        ...
        p1 = Provider(
            ...
            enabled=lambda: True,
            generate=MagicMock(
                side_effect=ProviderError("Rate limit exceeded", kind="rate_limit")
            ),
        )
        p2 = Provider(
            name="mock_fail_2",
    ...
    def test_think_falls_back_on_hf_exception(self):
        ...
        self.assertTrue(
            fallback_called, "Should fall back to self._generate for primary response"
        )
    ...
    def test_think_falls_back_on_legacy_gemini_exception(self):
        ...
        with patch.dict(
            sys.modules,
            {
                "google.generativeai": mock_legacy_genai,
                "local_genai_mock": mock_legacy_genai,
            },
        ):
            # Initialize DriftBrain
            brain = DriftBrain()
            ...
            for call in brain._generate.call_args_list:
                args, kwargs = call
                if (
                    args[0] == brain.primary_model_name
            ...
            self.assertTrue(
                fallback_called,
                "Should fall back to self._generate for primary response",
            )
