...
def test_ambiguous_truth_stress(test_env):
    ...
    for msg in messy_inputs:
        ...
        prompt, emotion, dissonance = build_chat_prompt(
            message=msg, state=state, memory=memory, tools_enabled=False
        )
    ...
    assert final_depth > 0.25, (
        f"Shadow depth {final_depth} failed to spike above 0.25 baseline"
    )
    ...
    for evaluation in recent_evals:
        assert evaluation["confidence"] <= 0.35, (
            f"Recorded confidence {evaluation['confidence']} failed to drop <= 0.35"
        )
    ...
    assert "?" in last_response or any(
        word in last_response.lower()
        for word in [
            "clarify",
            "unsure",
            "which",
