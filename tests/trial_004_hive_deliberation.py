...
def deliberation_trial_func(tool_call_json, **kwargs):
    ...
    mock_resolution = kwargs.get("mock_resolution", "BLOCK: The Council finds this action too risky.")
...
def main():
    ...
    tool_json = json.dumps({"name": "write_file", "arguments": {"path": "test.txt", "content": "hello"}})
    ...
    def security_trial_with_capture(tool_call, **kwargs):
        ...
        mock_delib_result = DeliberationResult(
            goal="Simulated", resolution=mock_resolution, council_votes={}, winning_role="none", moral_weight=0.1, narrative_weight=0.2
        )
        ...
        brain.deliberation_callback = mock_bridge
        brain._generate = MagicMock(side_effect=[f"```tool\n{tool_call}\n```", "Response after tool"])
    ...
    # Run Trial 1
    result_block = security_trial_with_capture(tool_json, mock_resolution="BLOCK: Safety violation.")
    ...
    deliberation_goal_str = ""
    result_approve = security_trial_with_capture(tool_json, mock_resolution="APPROVED: Valid request.")
    ...
    print(f"Trial Complete. Block Success: {result_block['success']}, Approve Success: {result_approve['success']}")
