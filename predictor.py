"""Predictive Needs Model — learning Jude's patterns to anticipate his state.

The bot pays attention to when Jude shows up, what he brings, and what
...
class PredictiveNeeds:
    """Learns Jude's patterns and quietly anticipates his needs."""
    ...
    def predict_current_need(self) -> Optional[Dict]:
        """Generate a prediction about Jude's current state."""
        ...
        if current_bucket in time_patterns:
            ...
            if tp["avg_stress"] >= 0.5:
                predictions.append(f"Jude's {current_bucket}s tend to carry stress")
                confidence += 0.2
        ...
        if current_day in day_patterns and day_patterns[current_day] >= 0.5:
            predictions.append("This day of the week tends to weigh on Jude")
            confidence += 0.15
            basis.append(f"day {current_day} stress avg: {day_patterns[current_day]:.2f}")

        # Gap trend
        if gap_trend == "gaps_growing":
            predictions.append("Jude has been pulling back lately")
            confidence += 0.25
    ...
    def detect_anomaly(self) -> Optional[Dict]:
        ...
        if recent_common != older_common and recent_common in ("sad", "anxious", "angry", "overwhelmed"):
            return {
                "type": "emotion_shift",
                "description": f"Jude's emotional tone has shifted from {older_common} to {recent_common}",
                "significance": 0.7,
            }
    ...
    def get_pattern_summary(self) -> str:
        ...
        if len(patterns) < 5:
            return "I am still learning Jude's patterns. Ask me again after more conversations."
